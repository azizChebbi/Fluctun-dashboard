  ...
  return (
    <div className=" h-screen overflow-hidden">
      <div className=" m-auto h-full grid grid-rows-[20%_1fr_30%] ">
        <div className={` flex items-center  ${sidebarIsSmall ? "justify-center" : "ml-[20%]"}`}>
          <div className=" ">
            {sidebarIsSmall ? (
              <img
                src={open}
            ...
            {sidebarIsSmall ? <img src={smallLogo} alt="logo" /> : <img src={logo} alt="logo" />}
          </div>
        </div>
          ...
          {tabs.map((tab, index) => {
            return (
              <Link
                to={tab.href}
                key={index}
                className={` my-6 gap-2 py-5 hover:bg-[#F8F8F8]${
              ...
              >
                <div className={`flex items-center ${sidebarIsSmall ? " justify-center" : "ml-[20%]"} `}>
                  ...
                  {!sidebarIsSmall && <span className=" text-xl font-medium text-right">{tab.label}</span>}
                </div>
              </Link>
            );
          })}
        </div>
        <div className={`flex items-center ${sidebarIsSmall ? "" : "ml-[20%]"} `}>
          <Link to="" className=" gap-2 w-full" onClick={logout}>
            <div className={` flex items-center ${sidebarIsSmall ? " justify-center" : ""} `}>
